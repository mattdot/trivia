...
Answer spaces have a **sweet-spot size: roughly 30–100 named items.** Big enough that blind guessing is hopeless and *reasoning has to do the pruning* — narrowing a recitable field down to 2–4 finalists is the whole game — yet small and familiar enough that the solver can actually walk the candidates. The great answer sets live here: the U.S. states (50), the countries a person can name (~100), an NFL or football league's teams, well-known universities, popular candies, common colors, the venues that have hosted a given event, a guessable count.

Both ends are weak, for opposite reasons:

- **Too small (a handful — the four majors, the eight planets, the seven big cats):** with nothing to prune, the solver is already standing at the finalist stage, so the question collapses to *recall the superlative* plus a 1-in-4 blind guess. A tiny set feels safely bounded but kills the reasoning. This is why "which of the four majors is oldest?" is flat, while "which **U.S. Open venue** has the highest average score?" is alive: the solver prunes ~50 courses down to Oakmont / Winged Foot / Shinnecock and bets. **When your answer set is a handful, widen it** — not "which major" but "which venue"; not "which of the great apes" but "which country has the most of them."
- **Effectively unbounded (red flag):** "which CEO," "which scientist," "which song," "which battle," "which painting," **"which animal."** No list to walk, so the only path is memorization. Rework or drop.

**Boundedness — and right-sizing — is checked first, and a rich dimension cannot rescue a badly-sized answer space.** This is the easiest gate to skip, because objectively-measurable dimensions are seductive: "fastest / loudest / strongest-biting animal" are all crisp, verifiable, reflex-defeating superlatives — and every one is *recall*, because the solver cannot enumerate the animal kingdom. Equally, a four-item set is recall of a different kind. Judge the *answer set's size and walkability* before you fall in love with the axis.

**The enumerability requirement is on the _answer set_, not the subject — this is the key, and the strongest fix.** A question's subject can be anything, however specific or unbounded — a single animal, a song, a battle; what must be walkable is the set the *answer* is drawn from. So the most powerful move for an unbounded answer is to **re-route the question so the answer comes from a bounded set, usually geography.** "Which animal is fastest?" has an unbounded answer (any creature); "which *country* has the most wild tigers?" keeps the rich animal subject but draws the answer from the ~195 countries the solver can recite — and reasons to (where do tigers live? → India). This beats bounding the *subject* to a famous subset ("which big cat is largest?"), because pinning the subject to seven cats forces the stalest superlatives — biggest (tiger), fastest (cheetah), all common knowledge — whereas "which country has the most leopards?" is fresh *and* a real bet. Let the interesting, specific, even unbounded thing be the *subject* the dimension measures, and hand the *answer* to a set the solver can walk.

Two fixes when you catch an unbounded answer, preferred first:
- **Re-route so the answer is a bounded set — and the bounded set can be _anything_ enumerable, not just countries.** Not "which scientist won X" but "which *country* has produced the most X winners"; not "which animal lives longest" but "which *country* protects the most of them." Countries are the common workhorse, but the answer set can be U.S. states, world cities, a league's teams, event venues, U.S. presidents, the alphabet, common colors, decades, even a guessable count. **Vary the answer set across the game** — the cluster rule applies here too: a set where every answer is a country is its own quiet monotony, the same failure as a set where every dimension was "sales." Reach for whichever enumerable set the question naturally lands on, and rotate through several over a round, with the rich subject riding along as what's being measured.

**See `answer-domains.md` (in this skill folder) for a catalog of right-sized answer sets and the _connectors_ that route any subject onto them.** The most versatile are the "universal routers" — **where** (countries, states, cities, venues), **when** (presidents, decades, months), and **how many** (a guessable count) — because nearly every subject has a where, a when, and a how-many, so an unbounded "which animal/song/war" almost always re-routes through one of them.

But **the route must not be decorative — it has to require knowledge of the _subject_.** A connector links subject → answer through a *bridge value* (a year, a place, a count); if the stem hands over that bridge, the subject is window-dressing. "Who was president during the *1980* Olympics?" is just "who was president in 1980" — the Olympics contributed nothing, because the year was given. Make the solver *derive* the bridge from subject knowledge ("the US boycotted the Summer Olympics only once — who was president then?"). The test: if stating the bridge directly reproduces the same question, the route is decorative (the same failure as a decorative filter). Aim for two non-trivial steps — subject → bridge, bridge → answer — with the stem leaking neither.
- **Bound the subject to an enumerable named group** (use when the subject itself must be the answer): "loudest of the **great apes**," "hardest-biting of the **big cats**," "most dangerous of **Africa's Big Five**." It works — one band's discography, WWII's major battles, a year's Nobel laureates — but beware that a tiny subject-set forces warhorse superlatives (biggest/fastest), so prefer re-routing the answer when you can.
...
**The best-without slicer (the "consolation crown") — the single most generative pattern.** A celebrated special case: **"most [A] among those that have never [B]"**, where B is the famous, defining achievement. "Most wins among college football programs that have never won a national title" → West Virginia. The magic is that B's filter **deletes every household name at once** — every champion is gone by definition — so the solver mentally crosses off the entire winners' list and hunts for the best of the overlooked rest. The answer is reliably reasonable-but-surprising: a genuine heavyweight that just never got the crown. It travels to any topic with a defining achievement: most career points without an NBA title; most domestic league titles without a European Cup; most populous country never to host the Olympics; most Oscar nominations without a win; biggest company never to top its industry. Reach for it often. Build it cleanly: A should be a countable record, B a single famous credential, and verify that the leftover leader is correct (the filter must actually exclude everyone more obvious).
...
- **Condition-qualified records.** "Largest country by area" is a gimme; adding a filter — "largest country completely surrounded by just one other" — makes it hard yet still reasonable: the solver pictures the map and tests candidates. Two cautions, both learned from play:
  ...
  - **The filter has to create real competition, not just relabel the obvious giant.** "Most populous country that drives on the left" technically applies a filter, but the answer (India) is still blindingly obvious — the qualifier narrowed the field without making the finalists hard to separate, so there's no bet to place. A working filter leaves two-to-four contenders a reasoner genuinely can't rank with confidence. If the filtered answer is still the first name that springs to mind, the filter is decorative; tighten it, re-slice it, or pick a dimension where the leaders are close.
  - **Attribute-as-filter can rescue answer-kind — and it earns its keep by creating doubt, by _binding_ or by _head fake_.** When an interesting attribute only ranks to a generic *type* (caffeine → "dark chocolate," not a brand), demote it to a *filter* and rank on an axis that lands on the primed answer-kind: "best-selling candy *that contains caffeine*" resolves to a brand. Such a filter is worth keeping if it makes the answer non-obvious in either of two ways:
    - **Binding** — it moves the answer off the unconditional leader. "Best-selling *sugar-free* candy" excludes every top seller and forces a new answer.
    - **Head fake** — it misdirects. "Contains caffeine" sends the solver hunting for energy candies and coffee chews before the click — *all chocolate has caffeine* — lands them back on the best-selling chocolate (a real Reese's / Snickers / M&M's bet). The answer equals the unfiltered best-seller, yet the solver had to see through the misdirection to trust it.
    The true failure — "most populous country that drives on the left" → India — is *neither*: nothing is excluded **and** no trap is set, so the answer is just the first name that springs to mind. The test is not "does the filter change the answer," it's **"is the filtered answer still the obvious first guess?"** If yes, it's decorative; if the solver must bind *or* see through a fake to get there, it's a real question. (A head fake only works if the misdirection is genuine — don't mistake a filter every solver reads straight through for a fake.)
...
Quantified targets (≥75% unique dimensions, 8–10 archetypes) are necessary but gameable — a set can hit them and still *feel* repetitive. **Vary the _kind_ of dimension, not just the measure — and watch the _category_ above the kind.** A countries set can pull every question from physical geography and population and still pass the unique-dimension count, yet it reads as monotonous — all maps and headcounts. Deliberately mix in economic (trade, output, currency), cultural and behavioral (what people consume, drive, watch, eat), historical, linguistic, sporting, and infrastructural axes. The dimension a player finds *refreshing* is usually the one that breaks the topic's default register — for countries, "coffee drunk per person" lands harder than yet another elevation or coastline. Aim for several such off-register dimensions per set.

**Beware the spec-sheet trap.** Topics with an obvious property table — chemical elements (density, melting point, conductivity, reactivity…), planets (mass, orbit, moons…), cars, athletes' stat lines — tempt you to draw every dimension from that one table. Twelve different *properties* are still twelve readings off the **same category** (intrinsic physical property), and the set will feel flat no matter how unique each property is. The escape is to enumerate genuinely orthogonal *categories* and force the set to span them: for elements, that's economics (which metal is priciest, most produced), biology (which is most abundant in blood or body), and physical extremes — not just one more physical constant. Before finalizing, list your dimensions and check that **no single super-category owns more than about a third of the set.** If they're all spec-sheet readings, you haven't found the topic's real range yet.

**Run an explicit cluster check — labels lie.** You can name a dozen "different" dimensions that all collapse to one quantity. After drafting the grid, group the dimensions by the *underlying thing they measure*, abstracting upward, and confirm no cluster owns more than about a third. The tell is a shared parent: "US sales, global sales, exports, per-capita consumption, production volume, seasonal sales" are six labels for **commercial scale**; "density, melting point, conductivity, reactivity" are four labels for **intrinsic physical property**. And the spec sheet that traps you isn't always physics — for consumer-product topics (candy, soda, cars, sneakers, fast food) it's almost always **money**: the generator locks onto best-seller and biggest-brand rankings exactly the way it locks onto the periodic table, and a set that looks varied ("sales vs revenue vs exports vs consumption") is one cluster in disguise. Breaking out means deliberately mining the non-commercial axes the topic actually has — ingredients and composition, physical structure, sensory extremes, history, design — even though they are harder to find and to verify. If after a genuine search those axes are thin, **that scarcity is the finding**: say the topic is ranking-poor off its dominant axis, and lean on the few real off-axis rankings rather than padding with six more sales questions.

**But category diversity must stay inside the ranking mechanic — don't trade a flat set for a recall set.** The danger when you flee the spec sheet is landing in pure trivia. "Which element was discovered in the Sun?", "which element did Curie name after Poland?", "which metal is named after a goblin?", "which metal's poisoning gave us 'mad as a hatter'?" are *category-diverse* but they are **memorized facts, not rankings** — there's no listable field to prune, just recognition. The off-register dimension must still be a **superlative with a reasoning path.** The reframe is usually small: "which metal is in your blood?" (recall) → "which metal is *most abundant* in your blood?" (a ranking you can reason — iron, zinc, copper, calcium — and bet on). If a category genuinely can't be cast as a ranking — naming, etymology, one-off discovery events — it isn't a dimension for this game; **cut it, don't smuggle recall in under the banner of variety.** Some topics (elements among them) have a narrower true-ranking range than they first appear, and the honest fix is to lean on the rankings that exist or pick a richer topic — not to pad with trivia.

Two traps that travel with this one: **open-ended time filters** — "once prized above gold" means *at any point ever*, an unbounded and arbitrary window; anchor to now or to a named era. And **arbitrary exclusions to force an answer** — "most reactive metal, ignoring francium because it's too rare" is the same gerrymander as fencing out Monaco for the GDP question. If the honest ranking has an inconvenient leader, change the dimension; don't fence the leader out.

So also check felt variety directly: for each question after the first, compare it to the previous 3–5. If it shares a **dimension**, answer family, statistic type, obvious leaderboard, source, or wording shape ("most wins," "largest total," "oldest," "first," "largest margin") with a recent neighbor, swap in a different subtopic and archetype. Repeated dimensions are the most common culprit — if two nearby questions both come down to "which is biggest" or "which is oldest," that's a dimension collision even when the answer sets differ. Make the first five questions visibly different from one another, on five different dimensions.
...
**Also keep out the answer's own discriminating figure** — the very number, year, or measurement that singles it out. "Which country broke away *in 2011*" and "which country has *over a quarter-million* islands" both leak the fact the player is supposed to reason toward; they match the number instead of earning it. Name the category ("the world's newest nation," "the country with the most islands"), never the value. If the stem needs a threshold to be well-posed, make it a *boundary the solver reasons against* (e.g. "with no coastline at all"), not the answer's defining stat.

**Don't reveal the size of the candidate set.** "The *only two* countries that are doubly landlocked — which is larger?" hands over that the field is tiny, which is itself a strong clue. State the property ("which doubly-landlocked country is larger by area?"), not the count.

**A "more than [named X]" boundary is also a giveaway** — it tells the solver X is not the answer. "More first-language Spanish speakers than *Spain* itself" quietly eliminates the most tempting guess. State the threshold without naming the subject it rules out, or drop the comparison and let the superlative stand on its own.

**Don't identify the answer by its single most iconic use, nickname, or association.** "Which gas, lighter than air, fills *floating balloons*?" → "balloon gas" is colloquially *helium*; the clause is the answer in disguise, so the solver recognizes rather than reasons. Same defect as naming the figure: it collapses the bet. Describe the answer by a *property the solver must test* (lighter than air **and** won't burn — which forces helium over hydrogen), not by the famous thing it's known for. But indirection must not create ambiguity — every stem must stay well-posed (gate step 3: exactly one *correct* answer, even though several finalists may look plausible to the solver).
...
1. **Survey the library first.** Run `python scripts/stats.py` to see which questions and dimensions are already used. The hard rule is **never repeat a question**; reusing an *answer* in a *different* question is fine. So treat already-asked questions as off-limits, and heavily-used dimensions as low-priority, when you plan the grid, so the new set genuinely extends the library rather than overlapping it.
...
3. **Check it:** `python scripts/check_uniqueness.py <path-to-set>.json`. A non-zero exit means a **hard collision** — a **question** whose stem repeats one already in the library (or within the set), or a single **answer used more than ~8% of this run**. Reusing an answer that appears in another set is allowed (reported as info). Regenerate just the colliding questions, then re-check. Near-duplicate stems and dimension-variety issues come back as warnings to weigh.
...
The skill still governs *how* questions are built (reasoning-driven, ≥75% unique dimensions within the set, well-posed, sourced). Library mode adds the cross-game guarantee on top: the scripts enforce that **no question is ever reused** (answers may recur in new questions, capped at ~8% within any one run), and `stats.py` keeps the dimensional spread honest as the library grows.

## Final checklist

Before delivering, confirm:

- [ ] Every answer is drawn from a **right-sized walkable set** (gate step 1) — ideally ~30–100 named items, big enough that reasoning must prune the field; not an unbounded "which animal/song/person" space, and not a too-small handful (4 majors, 7 cats) where the answer is just recalled. Widen tiny sets ("which major" → "which venue"); the answer sets vary across the round.
...
- [ ] Every dimension — including off-register ones — is a **ranking with a reasoning path**, not a memorized identity/event/naming fact; no open-ended "ever" time windows; no leader fenced out to force the answer.
- [ ] **Cluster check passed:** dimensions grouped by the quantity they really measure, no super-category over ~1/3 — and for consumer-product topics, the set isn't secretly all "commercial scale" (sales/revenue/exports/consumption).
- [ ] Any flagged-contested answer has a **reveal that teaches the contest** (names the rival answer, the figures, and why sources split).
