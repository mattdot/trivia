...
**Beware the spec-sheet trap.** Topics with an obvious property table — chemical elements (density, melting point, conductivity, reactivity…), planets (mass, orbit, moons…), cars, athletes' stat lines — tempt you to draw every dimension from that one table. Twelve different *properties* are still twelve readings off the **same category** (intrinsic physical property), and the set will feel flat no matter how unique each property is. The escape is to enumerate genuinely orthogonal *categories* and force the set to span them: for elements, that's discovery history (helium found in the Sun before Earth), naming/etymology (cobalt from a folklore goblin), human use and economics (aluminium once costlier than gold), biology and health (iodine and goitre), culture (mercury and "mad as a hatter") — not just one more physical constant. Before finalizing, list your dimensions and check that **no single super-category owns more than about a third of the set.** If they're all spec-sheet readings, you haven't found the topic's real range yet.
