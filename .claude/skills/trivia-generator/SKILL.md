...
**The template:** `assets/field-guide.html`. It's a "topographic field guide" design — drafting-paper tones, a survey benchmark medallion carrying each question's number — built around the core interaction: **each question is a large tap target that reveals only its own answer**, so the player reads, thinks, then taps. A quiet Reveal-all / Hide-all control and a live "revealed N of M" count sit in a sticky bar for convenience, but per-question tapping is the point — nothing reveals until the player chooses. It is mobile-first and tuned for phone readability (17px base, ~1.6 line-height on answer text, full-width answer column with block labels, ≥44px tap targets, visible keyboard focus, reduced-motion and dark-mode support). It uses only system fonts and no browser storage, so it is fully self-contained and renders cleanly inside the Claude app.

**How to render:**
1. Copy `assets/field-guide.html` to a working file.
2. Build the question data as comma-separated JS object literals and substitute them for the `/*__QUESTIONS__*/` marker. The marker already sits inside the template's `const QUESTIONS = [ … ];` brackets, so you insert the array's *contents* (the objects, comma-separated) — don't re-add the surrounding `[ ]`. Left unsubstituted, the page renders a harmless empty state. Each item is an object with keys `cat, q, state, detail, why, more, source` — map directly from the plain-text fields (`Answer:` → `state` + `detail`, `Why it's interesting:` → `why`, `Follow-on:` → `more`, `Source:` → `source`). `cat` is a short two-part eyebrow naming the broad domain and sub-dimension, e.g. `"Nature · Elevation"` — a light hint that must not reveal the answer or single out the one finalist.
...
4. Insert text via the data array only — the template assigns it with `textContent`, so apostrophes, em-dashes, degree signs, and `&` are safe; never paste question text straight into HTML.
...
The template's rendering logic is generic, so it works for **any** topic, not just the one it was first built for — only the `QUESTIONS` array and the four masthead strings change.
