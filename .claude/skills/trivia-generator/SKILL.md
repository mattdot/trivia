...
- **Condition-qualified records.** "Largest country by area" is a gimme; adding a filter — "largest country completely surrounded by just one other" — makes it hard yet still reasonable: the solver pictures the map and tests candidates. Two cautions, both learned from play:
  ...
  - **Attribute-as-filter can rescue answer-kind — but the filter must still _bind_.** When an interesting attribute only ranks to a generic *type* (caffeine → "dark chocolate," not a brand), you can demote it to a *filter* and rank on an axis that lands on the primed answer-kind: "best-selling candy *that contains caffeine*" resolves to a brand. The trap: the filter has to **move the answer off the unconditional leader.** Caffeine doesn't — the overall best-selling candy (Reese's) already contains it, so "best-seller with caffeine" = "best-seller," a decorative filter. Contrast "best-selling *sugar-free* candy," which excludes every top seller and forces a genuinely new answer. Test before using: does the filtered answer differ from the unfiltered one? If not, the attribute belongs in the *reveal* as a surprising fact, not in the stem as a filter.
