...
- **Attribute-as-filter — binding OR head fake.** User idea: rescue the caffeine angle as "highest-selling candy that contains caffeine" (filter = caffeine, rank = sales → brand answer). I first rejected it as decorative (answer = unfiltered best-seller, Reese's). User corrected: it's a **head fake** — "the problem with drives-on-the-left was that it was too obvious for India, it wasn't a head fake." *Rule (corrected):* the test isn't "does the filter change the answer," it's "is the filtered answer still the obvious first guess?" A filter earns its place by **binding** (sugar-free → new answer) OR by **head fake** (caffeine → solver hunts energy candies, then realizes all chocolate has caffeine → best-selling chocolate). Drives-on-left/India fails because it's neither — obvious, no misdirection. Don't conflate non-binding with decorative.

## Batch 8 (animals) — the unbounded-answer-set trap (foundational)

The questions were individually good — verified, objective metrics, reflex-defeaters, a clean cluster spread — and the topic *still* failed. User: "the set of answers is not bounded enough to make it guessable. it's traditional trivia."

- **Anti — rich dimension over an unbounded answer set.** "Fastest / loudest / strongest-biting animal" are crisp and verifiable, but the solver can't enumerate the animal kingdom, so they're recall, not prune-and-bet. *Rule:* boundedness of the ANSWER SET is gated first and independently; an objectively-measurable dimension cannot rescue an unbounded set. Natural-kingdom topics (animals, plants, dinosaurs) and "which song/battle/person" are unbounded by default. The seduction is that the dimension feels so clean you skip the walkability check.
- **The fix — bound to an enumerable named subset.** "Loudest animal" → "loudest of the great apes"; "strongest bite" → "hardest-biting big cat" (jaguar, over ~7 walkable species); "deadliest" → "most dangerous of Africa's Big Five." Generalizes: one band's discography, WWII's major battles, a year's Nobel laureates. Once the solver can recite the candidates, the mechanic returns.

## The capstone insight — enumerability is on the ANSWER SET, not the subject

After the bounded-subject "big cats" set still read as common-knowledge (biggest → tiger, fastest → cheetah), the user reframed it: "the bounded set doesn't have to be the subject. what country has the most leopards? would be a fine question."

- **The key move.** The thing that must be walkable is the set the *answer* is drawn from — the *subject* can be anything (a single animal, even an unbounded one). So re-route the question: "which animal is fastest?" (unbounded answer) → "which country has the most wild tigers?" (answer ∈ countries; subject = tigers). This is *stronger* than bounding the subject to seven cats, because pinning the subject forces the stalest superlatives, while "which country has the most leopards" is fresh AND a real bet.
- **Don't lock on countries (cluster rule, one level up).** User: "don't lock just on countries. it could be any reasonably enumerable set." The answer set can be continents, U.S. states, the planets, a count ("how many can roar?" → four), the roster itself — and a good game *rotates* through several. All-countries is the same quiet monotony as all-sales.
- **Filed demonstration** (`library/sets/2026-06-16-big-cats.json`): big cats as subject; answers drawn from countries (India/Tanzania/Russia), continents (Australia/Asia), the cats themselves (cheetah), and a count (four). Also surfaced the metric-units bug: "pound-for-pound bite" must report a *relative* figure, not an absolute PSI that contradicts the claim.

## Batch 9 (golf) — answer sets have an ideal SIZE (~30–100)

My golf set rotated six answer sets and passed the variety audit — and was still mostly recall. Two lessons:

- **Variety is gameable; I optimized the metric I could measure.** Hitting "6 sets / no lock" doesn't make questions good — half were factoids (14 clubs, Scotland, Europe) that are bounded *and* recall. Bounding the answer set is necessary, not sufficient: each question still needs a reasoning path and genuine doubt.
- **The decisive fix — right-size the answer set.** User: "Four possible answers is too small. The ideal enumerable set is probably 30–100 items… states, countries people can name, popular candies, football teams, universities, common colors, guessable counts." A handful (4 majors, 7 cats, 8 planets) leaves nothing to prune → recall + 1-in-4 guess. The model golf question routes to a right-sized set: **"which U.S. Open venue has the highest average score?" → Oakmont** (prune ~50 courses). Likewise "which U.S. state has the most golf courses?" → Florida (50), "which country besides the U.S. has the most major champions?" → Scotland (~100). Widen tiny sets: "which major" → "which venue."

## Batch 10 (Olympics stress test) — realized difficulty, and the decorative route

Routing one topic (Olympics) through 8 domains demonstrated the catalog's reach but the set came out easy. Two lessons:

- **Realized difficulty ≈ (field size to prune) × (how non-obvious the answer is).** A right-sized set gives only *potential* difficulty; if the answer is the canonical/famous one (Athens, USA, ~200), the solver skips the pruning and recalls it, and the difficulty collapses. The reflex-defeater — picking the fact whose answer *isn't* the first name to mind — is the knob that converts a big field into a hard question.
- **The decorative route (user catch).** "Who was president during the *1980* Moscow Olympics?" — the Olympics is window-dressing; it's just "who was president in 1980" because the stem gave away the bridge value (the year). *Rule:* a route earns its keep only if the solver must derive the bridge (year/place/count) from *subject* knowledge — never state it. Test: if stating the bridge directly reproduces the question, the route is decorative. Best routed questions take two non-trivial steps (subject → bridge → answer), leaking neither.

## Batch 11 (college football) — the difficulty knob, and its ceiling

Built two versions to test whether difficulty is controllable. Batch 1 (canonical answers) came out mostly easy; batch 2 put a reflex-defeated answer on every question and came out uniformly hard. Confirms: **right-sized domain = fair; reflex-defeater (or non-decorative route) = hard.** Knobs verified — Michigan (most wins, not Alabama), USC (most Heismans, not Notre Dame), Paterno (most coaching wins, not Saban/Bryant), 222 (highest score), Texas A&M (biggest SEC stadium).

- **The reasoning-heuristic test (user correction).** I first tagged "most coaching wins → Paterno" as obscurity-hard ("you must know the number"). Wrong: career wins = longevity × success, so you reason *"who coached forever and won?"* → **Paterno and Bobby Bowden** as the two finalists. It's a fair bet, not recall. *Rule:* before calling a record "obscurity," look for the **reasoning heuristic** — longevity → career totals, dynasty/recency → titles, recent expansion → biggest stadium, "guess high" → blowout score. If a heuristic reaches the finalists, the question is reasoning-fair even when the raw stat is obscure. True obscurity is only the **exact unreasonable figure** ("how many wins did Paterno have?" → 409) or a fact with **no heuristic at all**.
- **The ceiling, corrected.** So most of the uniformly-hard set is actually fair (longevity, blue-blood, post-war-era, guess-high heuristics); only a question with *no* heuristic is genuinely too hard. Still vary difficulty across a real set — but "hard" is fine wherever a heuristic exists, which is more often than I'd credited.
- **The master difficulty dial: the slicer (user).** "What you're missing is slicers/filters to remove the obvious." The controllable way to harden any easy question is to add a filter that *removes the obvious answer*, forcing the solver one tier down — "most Olympic golds" → USA becomes "most golds among never-host countries"; "first modern Olympics" → Athens becomes "first in the Southern Hemisphere" → Melbourne. *Rule:* difficulty isn't "canonical (easy) vs weird (obscure)" — it's a dial, and the slicer turns it. Each tighter slice peels off another obvious tier, stays reasoning-based (apply filter, re-rank), and is the constructive way to manufacture a reflex-defeater. Same guardrails: bind, natural-not-gerrymandered, no giveaway.

## The "best-without" slicer (consolation crown) — a favorite, per the user

User on "most all-time wins among programs that never won a national title → West Virginia": "a fantastic question… we want more like that." *Pattern:* "most [A] among those that have never [B]", B = the famous defining achievement. The filter deletes every household name at once, so the solver crosses off the whole winners' list and finds the best of the rest — reliably reasonable-but-surprising. *Rules for building it:* route the answer to a right-sized set (franchise/program/country, never an individual); A is a countable record, B a single famous credential; verify the leftover leader is actually correct. Travels everywhere: most NBA wins without a title, most domestic titles without a European Cup, most populous country never to host the Olympics, most Best Picture noms without a win. Now a named pattern in SKILL.md.
