...
- **The ceiling (new lesson):** cranking the reflex-defeater to *max on every question* tips some from reasoning-hard into **obscurity-hard** — Paterno's win total and Kyle Field's capacity are facts you must *know*, not reason to, which is exactly what "difficulty from reasoning, not obscurity" forbids. *Rule:* don't max the knob on the whole set. The ideal is a difficulty *spread* — mostly fair-reasoning mediums, a couple of hard reflex-defeaters — per "vary how much doubt remains across the set." Uniformly-hard is a diagnostic, not a target.
