...
- **Attribute-as-filter, with a binding test.** User idea: rescue the caffeine angle as "highest-selling candy that contains caffeine" (filter = caffeine, rank = sales → brand answer). Good *technique* — but caffeine doesn't bind: the overall best-seller (Reese's) already has caffeine, so it's a decorative filter that collapses to "best-seller." *Rule:* attribute-as-filter is a real tool to fix answer-kind, but the filter must move the answer off the unconditional leader (cf. "best-selling *sugar-free* candy"). If it doesn't, the attribute belongs in the reveal as a surprising fact.
